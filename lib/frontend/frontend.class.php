	...
	<div class="WooZone-cart-msg"></div>
		...
		public function cross_sell_box( $atts ) {
			...
			if ( isset($products['status'], $products['rows']) && 'valid' == $products['status'] && !empty($products['rows']) ) {
				...
				$how_many = $how_many + 1; // add 1 fake return products, current product
				...
				return isset($products['msg']) && !empty($products['msg']) ? $products['msg'] : '';
			}
		}

		public function _cross_sell_get_similarity_prods( $asin, $return_nr=3, $force_update=false ) {
			$ret = array('status' => 'invalid', 'rows' => array() );

			$retProd = array();

			$return_nr = $return_nr + 1; // add 1 fake return products, current product
			...
			if ( $cache_request != "" && count($cache_request) > 0 && $force_update === false ) {
				...
				// force refresh ( nb_tries is zero for old requests - which didn't mange variable products )
				if ( isset($cache_request['nb_tries']) && empty($cache_request['nb_tries']) ) {
					$force_update = true;
				}
				...
				if ( isset($cache_request['add_date'])
					...
					return array_merge( $ret, array('status' => 'valid', 'rows' => array_slice( $products, 0, $return_nr )) );
				}
			}
			...
			if ( $cache_request != "" && count($cache_request) > 0 ) {
				$updateQuery = "update " . $db->prefix . "amz_cross_sell" . " set products = %s, nr_products = %s, is_variable = %s, nb_tries = nb_tries + 1 where 1=1 and ASIN = %s;";
			...
			return array_merge( $ret, array('status' => 'valid', 'rows' => array_slice( $retProd, 0, $return_nr )) );
		}
