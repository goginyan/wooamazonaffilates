...
if (class_exists('WooZoneTailSyncMonitor') != true) {
    class WooZoneTailSyncMonitor
    {
        ...
        */
		...
		static private $sync_options = array();
        
        const SQL_PRODUCTS_LIMIT_MAX = 5;
        ...
        */
        public function __construct()
        {
            ...
            $this->module = $this->the_plugin->cfg['modules']['synchronization'];
        ...
        private function init_sync_options() {
...
<?php
        ...
        private function sync_settings( $pms=array() ) {
        	extract($pms);
            ob_start();
...
<?php
        ...
        private function get_products( $pms=array() ) {
            global $wpdb;
            ...
            foreach ($res as $id => $val) {
                ...
                //if ( $nbprod >= self::SQL_PRODUCTS_LIMIT_MAX  ) { $isBreakLimit = true; break 1; }
        ...
        private function get_product_variations( $pms=array() ) {
            global $wpdb;
        ...
        private function get_next_product( $last_id=0 ) {
            global $wpdb;
        ...
        private function row_view_html( $row, $is_child=false ) {
		...
		private function get_interface_max_products( $use_pag=false ) {
         ...
         */
        private function get_thumb_src_default() {
            return '##default##';
        }

        private function get_thumbs( $currentIds=array(), $size='thumbnail' ) {
            global $wpdb;
            ...
            foreach ($thumbsId as $key) {
				...
				if ( strpos( $thumbs["$key"], WooZone::$amazon_images_path ) ) {
        ...
        private function get_thumb( $meta, $size='medium', $pms=array() ) {
        ...
        private function get_thumb_src( $meta, $size='medium' ) {
        ...
        private function wp_get_attachment_url( $meta ) {
            $url = '';
        ...
        private function image_get_intermediate_size( $meta, $size='thumbnail' ) {
		 ...
		 * Pagination
		 */
		private function build_pagination_vars() {
		...
		private function get_pagination( $pms=array() )
		{
			extract($pms);
			extract( $this->build_pagination_vars() );
