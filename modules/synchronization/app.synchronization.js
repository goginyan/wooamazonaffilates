...
WooZoneTailSyncMonitor = (function ($) {
	...
	(function init() {
		...
		$(document).ready(function(){
            ...
            syncall_msg = maincontainer.find('#WooZone-content-area .WooZone-panel-header span.WooZone-panel-sync-all');
	...
	function syncSettings( el ) {
        ...
        WooZone.to_ajax_loader( "Saving synchronisation settings.." );
        ...
        jQuery.post(ajaxurl, data, function(response) {
            ...
            if( response.status == 'valid' ){
                ...
                mainsync.find( settings_wrapp ).append( response.form );
	...
	function syncProduct( row, callback )
	{
	    ...
	    if ( $.isFunction(callback) && syncall_status == 0 ) {
	        syncall_msg.html( lang.sync_now_stopped );
	        goto_next( row_first, {'time': 1000} );

            WooZone.to_ajax_loader_close();

            //loading.find('.WooZone-loading-text').html( lang.loading );
	...
	function next_to_sync( row, callback ) {
        if ( row.length > 0 ) { // next row
            ...
            syncall_msg.html( lang.sync_now_finished );
    ...
    function open_product_variations( row, status, callback ) {
        ...
        var make_request = function( action, params, callback ){
            ...
            if ( typeof callback == 'undefined' ) WooZone.to_ajax_loader( lang.loading );
    ...
    function triggers()
    {
        ...
        maincontainer.on('click', '.WooZone-sync-filters span.right button.sync-all', function(e){
            ...
            if ( rows.length <= 0 ) {
                syncall_msg.html( lang.no_products );
                return false;
            }
			...
			WooZone.to_ajax_loader( '<strong>' + lang.sync_now_inwork + '</strong>' + '<button>' + lang.sync_now_stop_btn + '</button>', $('.WooZone-sync-table') );
            //loading.find('.WooZone-loading-text').html( '<strong>' + lang.sync_now_inwork + '</strong>' + '<button>' + lang.sync_now_stop_btn + '</button>' );
            //loading.show();
            ...
            */
            ...
            syncProduct( rows.eq(0), syncProduct );
        });
        
        // stop sync all
        loading.on('click', '.WooZone-loading-text button', function(e){
            e.preventDefault();
