...
if(class_exists('WooZone') != true) {
	class WooZone {
		...
		public $ss = array(
			...
			'max_products_demo_keys'			=> 9,
			
			// admin css cache time ( 0 = no caching )
			'css_cache_time'							=> 86400, // seconds  (86400 seconds = 24 hours)
			
			// amazon country shops where product is available - cache time ( 0 = no caching )
			'countries_cache_time'					=> 43200, // seconds  (86400 seconds = 24 hours)
		);
		...
		public function _attachment_url( $url='', $post_id=0 ) {
			...
			if ( !$this->is_prod_valid($post->post_parent) ) {
				return $url;
			}
		...
		public function demo_products_import_end_html()
		{
			$html = array();
		...
		public function frontpage()
		{
			...
			add_filter( 'woocommerce_product_gallery_attachment_ids', array($this, 'amz_product_gallery_attachment_ids'), 10, 2 );
		...
		public function amz_product_gallery_attachment_ids( $gallery_ids, $product ) {
			...
			if ( !is_product() || !$this->verify_product_isamazon($product_id) ) {
				return $gallery_ids;
			}
				...
				if( $theme_name != "Kingdom - Woocommerce Amazon Affiliates Theme" ){
					...
					if( !isset($current['theme']) || !$current['theme'] ){
						...
						$_errors = array('
							<p>
								<strong>
								For maximum usability and best experience with our WooZone Plugin we recommend using the custom themes - <a href="http://themeforest.net/item/kingdom-woocommerce-amazon-affiliates-theme/15163199?ref=AA-Team" target="_blank">Kingdom </a> or <a href="http://codecanyon.net/item/the-market-woozone-affiliates-theme/13469852?ref=AA-Team" target="_blank">The Market </a> available on Codecanyon.
								</strong>
							</p>
							<p>
								<strong>
									<a href="http://themeforest.net/item/kingdom-woocommerce-amazon-affiliates-theme/15163199?ref=AA-Team" target="_blank">Grab this theme</a> | <a class="dismiss-notice" href="' . ( admin_url( 'admin-ajax.php?action=WooZoneDismissNotice&id=theme' ) ) . '" target="_parent">Dismiss this notice</a>
								</strong>
							</p>
						') ;
					}
		...
		*/
		...
		public function get_country_perip_external( $return_field='country' ) {
			...
			// DEBUG
				...
				if ( isset($_SESSION['WooZone_country']) ) unset($_SESSION["WooZone_country"]);
			}
...
;";
		...
		public function update_db()  {
			...
			// installed version less than 9.0 / 8.4.1.3
			...
			if (1) {
				...
				if ($this->db->get_var("show tables like '$table_name_report_assets'") == $table_name_report_assets) {
					$this->_update_db_tables(array(
						'table'			=> $table_name_report_assets,
						'queries'		=> array(
							'image_sizes'				=> array(
								'main' 			=> "ALTER TABLE " . $table_name_report_assets . " %s COLUMN `image_sizes` TEXT NULL;",
								'verify'			=> "SHOW COLUMNS FROM " . $table_name_report_assets . " LIKE 'image_sizes';",
								'field_name'	=> 'image_sizes',
								'field_type'	=> 'text',
							),
							'download_status'		=> array(
								'main' 			=> "ALTER TABLE " . $table_name_report_assets . " %s COLUMN `download_status` VARCHAR(20) NULL DEFAULT 'new' COMMENT 'new, success, inprogress, error, remote';",
								'verify' 			=> "SHOW COLUMNS FROM " . $table_name_report_assets . " LIKE 'download_status';",
			...
			//if ( version_compare( $current_db_version, '9.0.3.3', '<' ) ) {
			if (1) {
				$table_name_cross_sell = $this->db->prefix . "amz_cross_sell";
				if ($this->db->get_var("show tables like '$table_name_cross_sell'") == $table_name_cross_sell) {
					$this->_update_db_tables(array(
						'table'			=> $table_name_cross_sell,
						'queries'		=> array(
							'is_variable'				=> array(
								'main' 			=> "ALTER TABLE " . $table_name_cross_sell . " %s COLUMN `is_variable` CHAR(1) NULL DEFAULT 'N';",
								'verify'			=> "SHOW COLUMNS FROM " . $table_name_cross_sell . " LIKE 'is_variable';",
								'field_name'	=> 'is_variable',
								'field_type'	=> 'char(1)',
							),
							'nb_tries'		=> array(
								'main' 			=> "ALTER TABLE " . $table_name_cross_sell . " %s COLUMN `nb_tries` TINYINT(1) UNSIGNED NULL DEFAULT '0';",
								'verify' 			=> "SHOW COLUMNS FROM " . $table_name_cross_sell . " LIKE 'nb_tries';",
								'field_name'	=> 'nb_tries',
